...
class UNetModel(torch.nn.Module):
    ...
    def forward(self, x):
        enc1 = self.enc1(x)
        # print("enc1",np.shape(enc1.detach().cpu().numpy().squeeze()))
        pooled_enc1 = self.pool1(enc1)
        # print("pooled 1",np.shape(pooled_enc1.detach().cpu().numpy().squeeze()))
        enc2 = self.enc2(pooled_enc1)
        # print("enc2",np.shape(enc2.detach().cpu().numpy().squeeze()))
        pooled_enc2 = self.pool2(enc2)
        # print("pooled 2",np.shape(pooled_enc2.detach().cpu().numpy().squeeze()))
        enc3 = self.enc3(pooled_enc2)
        # print("enc3",np.shape(enc3.detach().cpu().numpy().squeeze()))
        pooled_enc3 = self.pool3(enc3)
        # print("poooled 3",np.shape(pooled_enc3.detach().cpu().numpy().squeeze()))
        enc4 = self.enc4(pooled_enc3)
        # print("enc4",np.shape(enc4.detach().cpu().numpy().squeeze()))
        pooled_enc4 = self.pool4(enc4)
        # print("pooled 4",np.shape(pooled_enc4.detach().cpu().numpy().squeeze()))
        center = self.center(pooled_enc4)
        # print("center",np.shape(center.detach().cpu().numpy().squeeze()))
        dec4 = self.dec4(torch.cat([enc4, center], 1))
        # print("dec4",np.shape(dec4.detach().cpu().numpy().squeeze()))
        dec3 = self.dec3(torch.cat([enc3, dec4], 1))
        # print("dec3",np.shape(dec3.detach().cpu().numpy().squeeze()))
        dec2 = self.dec2(torch.cat([enc2, dec3], 1))
        # print("dec2",np.shape(dec2.detach().cpu().numpy().squeeze()))
